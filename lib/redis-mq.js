...
class RedisStream {
  ...
  constructor(config) {
    ...
    this.redisVersion = config.redisVersion
    this.start(config)
  }

  start(config) {
    this.redisRead = config.client || new Redis()
  ...
  async xrange(streamKey = this.config.stream, start = '-', end = '+', count) {
    const query = [streamKey, start, end]
    if (count) query.push('COUNT', count)
    const res = await this.redisRead.xrange(query)
    return res.map(i => this.arr2item(i))
  }

  async xrevrange(streamKey = this.config.stream, start = '+', end = '-', count) {
    const query = [streamKey, start, end]
    if (count) query.push('COUNT', count)
   ...
   */
  async readByGroup({
                      id = '>', consumer = this.config.consumer, group = this.config.group,
   ...
   */
  async readPending({
                      start, end, group = this.config.group,
   ...
   * @param {string} stream channel-name
   */
  async read(ID = 0, count = 10, block, stream = this.config.stream) {
    let query = ['STREAMS', stream, ID]
    if (count) query = ['COUNT', count, ...query]
    // .concat(query)
    if (block) query = ['BLOCK', block].concat(query)
  ...
  async getStreamInfo(stream = this.config.stream, full, count) {
    ...
    if (full && this.redisVersion >= '6.0.0') {
      ...
      res['last-entry'] = this.arr2item(res['last-entry'])
    }
...
class RedisBase {
  constructor({ client, db = 0 } = {}) {
    this.client = client
    this.db = db
  }

  async serverInfo() {
    const txt = await this.client.info()
    ...
    //   pubsub_channels, pubsub_patterns } = info

    return { txt, dbs }
    // [info, dbs, txt, { redis_version, ...}]
  }
}
